...
export const createSong = async (req, res) => {
    try {
        ...
        if (req.body.artists) {
            // Note: We need the song ID first before creating TrackArtist associations.
            // The logic below tries to use 'song._id' which is undefined at this point.
            // We must create the song FIRST.
        }
        ...
        if (req.body.artists) {
            ...
            // Handle array if it's sent as JSON string or already parsed by express-urlencoded
...
export const updateGenre = async (req, res) => {
    try {
        ...
        const genre = await Genre.update(genreId, req.body);
...
export const UploadEpisodeToPodcasts = async (req, res) => {
    try {
        ...
        const podcast = await Podcast.findById(req.body.podcast_id);
        if (!podcast) {
            return res.status(404).json(new ApiError(404, "Podcast not found"));
        }
        if (!audioUrl?.url) {
